    ...
    <main class="content-studio-main">
        <header class="studio-header">
            <div class="studio-title-wrap">
                <a class="back-btn" href="detail-course.html">
                    <i class="bi bi-arrow-left"></i>
                </a>
                <div>
                    <p class="kicker">STUDIO NARASUMBER • PENYUSUNAN MATERI</p>
                    <h1>Visual Branding Architecture</h1>
                </div>
            </div>
        ...
        <section class="studio-layout">
            <div class="studio-left">
                <section class="panel panel-module active" data-panel="module">
                    <form id="moduleForm" class="module-form" action="{{ route('trainer.upload-materials') }}" method="POST"
                ...
                <section class="panel panel-quiz" data-panel="quiz">
                    <form id="quizForm" class="quiz-form" action="{{ route('trainer.save-quiz') }}" method="POST">
                        @csrf
                        <input type="hidden" name="courseId" value="{{ $courseId ?? 0 }}">
                        <div class="quiz-meta">
            ...
            <aside class="studio-right">
                <div class="validation-card">
                    <h3>ALUR VALIDASI QUIZ</h3>
                    <ol>
                        <li>
                            <span>1</span>
                            <div>
                                <h4>DRAFTING NARASUMBER</h4>
                                <p>
                                    Anda menyusun soal, opsi, dan kunci jawaban secara mandiri.
                                </p>
                            </div>
                        </li>
                        <li>
                            <span>2</span>
                            <div>
                                <h4>AUDIT ADMIN TRAINER</h4>
                                <p>
                                    Review kesesuaian soal dengan materi dan kelengkapan kunci.
                                </p>
                            </div>
                        </li>
                        <li>
                            ...
                            <div>
                                ...
                                <p>Kuis akan otomatis terbit di halaman LMS peserta.</p>
                            </div>
                        </li>
                    </ol>
                </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            dropzone.addEventListener("dragover", (e) => {
                ...
                dropzone.style.backgroundColor = "#e5ebf4";
            });
                ...
                if (uploadedFiles.length > 0) {
                    ...
                    fetch(moduleForm.action, {
                        ...
                        .then(data => {
                            if (data.success) {
                                alert(
                                    `Materi berhasil disubmit!\n${data.files.length} file telah diunggah untuk review.`,
                                );
                                uploadedFiles = [];
                                fileInput.value = '';
                                updateFileList();
            ...
            function renderQuestion(index) {
                ...
                <div class="q-head">
                  ...
                  <button type="button" class="delete-question" data-index="${index}">
                  ...
                  <div class="options-grid">
                    ${question.options
                      ...
                      <div class="option-container">
                        <button type="button" class="option-btn ${question.correctAnswer === optIndex ? "is-correct" : ""}" data-question-index="${index}" data-option-index="${optIndex}">
                          <i class="bi ${question.correctAnswer === optIndex ? "bi-check-circle-fill" : "bi-circle"}"></i>
                          <span>Opsi ${optIndex + 1}</span>
                        </button>
                        <input type="text" class="option-input" data-question-index="${index}" data-option-index="${optIndex}" placeholder="Masukkan jawaban..." value="${option}" />
                      </div>
                    `,
                        )
                        .join("")}
              ...
              `;
                ...
                // Find existing element or append
                const existingEl = questionsContainer.querySelector(
                    `[data-question-id="${question.id}"]`,
                );
                if (existingEl) {
                    existingEl.replaceWith(questionEl);
                } else {
                ...
                questionEl
                    ...
                    .addEventListener("input", (e) => {
                        quizQuestions[index].text = e.target.value;
                    });

                questionEl
                    .querySelector(".weight-input")
                    .addEventListener("input", (e) => {
                        quizQuestions[index].weight = parseInt(e.target.value) || 0;
                        updateTotalWeight();
                    });
                ...
                questionEl
                    ...
                    .addEventListener("click", (e) => {
                        ...
                        quizQuestions.forEach((_, i) => renderQuestion(i));
                        updateTotalWeight();
                    });
                ...
                questionEl.querySelectorAll(".option-btn").forEach((btn) => {
                    btn.addEventListener("click", (e) => {
                        ...
                        quizQuestions[qIdx].correctAnswer = oIdx;
                        renderQuestion(qIdx);
                    });
                });

                questionEl.querySelectorAll(".option-input").forEach((input) => {
                    input.addEventListener("input", (e) => {
                        const qIdx = parseInt(e.target.dataset.questionIndex);
                        const oIdx = parseInt(e.target.dataset.optionIndex);
                        quizQuestions[qIdx].options[oIdx] = e.target.value;
                    });
                });
            }
            ...
            function updateTotalWeight() {
                ...
                verifyStatusDisplay.style.background =
                    total > 0 ? "#2c237f" : "#ff6b6b";
            }

            // Click-to-Edit Handlers for Meta Boxes
            ...
            passingGradeBox.addEventListener("click", (e) => {
                const isHidden =
                    window.getComputedStyle(passingGradeInput).display === "none";
                if (isHidden) {
            ...
            passingGradeInput.addEventListener("blur", () => {
                let value = parseInt(passingGradeInput.value) || 70;
                value = Math.max(0, Math.min(100, value));
                passingGradeInput.value = value;
                passingGradeDisplay.textContent = value;
                passingGradeDisplay.style.display = "inline";
                passingGradeInput.style.display = "none";
            });

            passingGradeInput.addEventListener("keypress", (e) => {
                if (e.key === "Enter") {
                    let value = parseInt(passingGradeInput.value) || 70;
                    value = Math.max(0, Math.min(100, value));
                    passingGradeInput.value = value;
            ...
            quizForm.addEventListener("submit", (e) => {
                ...
                if (quizQuestions.length === 0) {
                    alert("Silakan tambahkan minimal 1 soal.");
                    return;
                }

                const allValid = quizQuestions.every(
                    (q) =>
                        q.text.trim() !== "" && q.options.every((o) => o.trim() !== ""),
                );

                if (!allValid) {
                    alert("Semua pertanyaan dan opsi harus diisi.");
                    return;
                }

                const quizData = {
                    passingGrade: parseInt(passingGradeInput.value),
                    totalWeight: quizQuestions.reduce((sum, q) => sum + q.weight, 0),
                    questions: quizQuestions,
                };

                // Add hidden inputs for form submission
                const form = document.getElementById('quizForm');
                const existingInput = form.querySelector('input[name="questions"]');
                if (existingInput) {
                ...
                const passingGradeInput2 = form.querySelector('input[name="passingGrade"]');
                if (passingGradeInput2) {
                    passingGradeInput2.remove();
                }

                const passingInput = document.createElement('input');
                passingInput.type = 'hidden';
                passingInput.name = 'passingGrade';
                passingInput.value = parseInt(passingGradeInput.value);
                form.appendChild(passingInput);

                // Submit the form
                form.submit();
            });

            // Add first question by default when quiz tab opens
            let quizInitialized = false;
            tabs.forEach((tab) => {
                if (tab.dataset.tab === "quiz") {
                    tab.addEventListener("click", () => {
                        if (!quizInitialized && quizQuestions.length === 0) {
                            addQuestion();
            ...
            // Initialize first question if starting on quiz tab
            if (initialTab === "quiz" && quizQuestions.length === 0) {
                addQuestion();
                quizInitialized = true;
            }
        });
