...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="mb-1 fw-bold">
            <i class="bi bi-plus-circle text-primary me-2"></i>Tambah Carousel
        </h4>
        <p class="text-muted mb-0">Tambah gambar carousel baru untuk media promosi</p>
    </div>
    <a href="{{ route('admin.carousels.index', ['location' => $location]) }}" class="btn btn-outline-secondary">
                ...
                <form action="{{ route('admin.carousels.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="location" id="location" class="form-select @error('location') is-invalid @enderror" required>
                                ...
                                @foreach($locations as $key => $label)
                                    <option value="{{ $key }}" {{ old('location', $location) === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-4">
                        <label for="image" class="form-label fw-semibold">
                            <i class="bi bi-image me-2"></i>Gambar <span class="text-danger">*</span>
                        </label>
                        <div class="preview-container" id="previewContainer">
                            <div id="previewPlaceholder">
                                <i class="bi bi-cloud-upload fs-1 text-muted mb-3 d-block"></i>
                                <p class="text-muted mb-2">Klik untuk memilih gambar atau drag & drop</p>
                                <p class="small text-muted">Format: JPG, PNG, GIF, WEBP (Maks. 5MB)</p>
                            </div>
                        ...
                        <div class="mt-2">
                            <small class="text-muted">
                                <i class="bi bi-info-circle me-1"></i>
                                <strong>Ukuran Optimal:</strong> 1920x600px (rasio 16:5) atau 1350x400px<br>
                                <strong>Rasio Aspek:</strong> 3.2:1 (lebar lebih panjang dari tinggi)
                            </small>
                        </div>
                        @error('image')
                    ...
                    <div class="d-flex justify-content-end gap-2 pt-3 border-top">
                        ...
                        <button type="submit" class="btn btn-primary shadow-sm">
                            <i class="bi bi-check-circle me-2"></i>Simpan Carousel
                        </button>
                    </div>
                </form>
            </div>
