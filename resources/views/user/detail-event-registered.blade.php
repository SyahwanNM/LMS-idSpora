...
<head>
    ...
    <style>
        ...
        .share .share-action svg {
            flex: 0 0 auto;
        }

        .share .share-action:hover {
            background: #f3f4f6;
        }

        .share .share-action:active {
            background: #e5e7eb;
        }
        ...
        .share .share-action--disabled {
            opacity: .45;
            cursor: not-allowed;
            pointer-events: none;
        }

        .share .share-list::-webkit-scrollbar {
            height: 0;
        }
        ...
        .resource-box .participant-resources .resource-card .resource-value p {
            margin-bottom: 0;
            line-height: 1.25;
        }
        ...
        #feedback-confirm-modal .modal-content {
            border-radius: 18px;
            overflow: hidden;
        }

        #feedback-confirm-modal .modal-header {
            padding: 1.1rem 1.1rem .75rem;
        }

        #feedback-confirm-modal .modal-body {
            padding: 0 1.1rem 1rem;
        }

        #feedback-confirm-modal .modal-footer {
            padding: .25rem 1.1rem 1.1rem;
            border-top: 0;
        }

        #feedback-confirm-modal .btn {
            border-radius: 12px;
            padding-top: .6rem;
            padding-bottom: .6rem;
        }

        #feedback-confirm-modal .modal-footer .btn {
        ...
        /* Add To Calendar button: make it smaller (size only, keep theme) */
        .add-calender button {
        ...
        .add-calender .ikon-calender-event {
...
<body>
    ...
    <main class="all">
        <div class="container-ungu">
            ...
            <div class="box-event-creator">
                ...
                <div class="add-calender">
                    @php
                        ...
                        $parseEvtTimeTop = function ($date, $raw) {
                            ...
                            if (preg_match('/\d{4}-\d{2}-\d{2}/', $rawStr)) {
                                try {
                                    return \Carbon\Carbon::parse($rawStr);
                                } catch (\Throwable $e) {
                                    return null;
                                }
                            }
                            if ($date) {
                                ...
                                try {
                                    return \Carbon\Carbon::parse($dateStr . ' ' . $rawStr);
                                } catch (\Throwable $e) {
                                    return null;
                                }
                            }
                    ...
                    @endphp
                    @if(!empty($datesParam))
                        <button class="" type="button" onclick="window.open('{{ $gcalUrl }}','_blank')"
                            title="Add to Google Calendar">
                            <svg class="ikon-calender-event" xmlns="http://www.w3.org/2000/svg" width="25" height="25"
                    ...
                    @else
                        <button class="" disabled title="Tanggal/waktu belum tersedia"
            ...
            <div class="event-title">
                ...
                @php
                    ...
                    $statusLbl = null;
                    if ($startDate) {
                        if ($nowLbl->lt($startDate))
                            $statusLbl = 'Segera Hadir';
                        elseif ($nowLbl->isSameDay($startDate))
                            $statusLbl = 'Berlangsung';
                        elseif ($nowLbl->gt($startDate))
                            $statusLbl = 'Telah Selesai';
                    }
        ...
        <div class="detail-box">
            <div class="detail-box-left">
                <img src="{{ isset($event) && $event->image_url ? $event->image_url : asset('aset/event.png') }}"
                    alt="{{ isset($event) ? $event->title : 'Gambar Event' }}"
                    onerror="this.src='{{ asset('aset/event.png') }}'">
                @php
                    $authUser = Auth::user();
                ...
                <div class="info-boxluar">
                    <div class="event-info-item">
                        <div class="event-info-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                                class="bi bi-calendar-date" viewBox="0 0 16 16" aria-hidden="true">
                                <path
                    ...
                    <div class="event-info-item">
                        <div class="event-info-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                                class="bi bi-clock" viewBox="0 0 16 16" aria-hidden="true">
                                <path
                    ...
                    <div class="event-info-item">
                        <div class="event-info-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                                class="bi bi-geo-alt" viewBox="0 0 16 16" aria-hidden="true">
                                <path
                                    d="M12.166 8.94c-.524 1.062-1.234 2.12-1.96 3.07A31.493 31.493 0 0 1 8 14.58a31.481 31.481 0 0 1-2.206-2.57c-.726-.95-1.436-2.008-1.96-3.07C3.304 7.867 3 6.862 3 6a5 5 0 0 1 10 0c0 .862-.305 1.867-.834 2.94" />
                    ...
                    <div class="event-info-item">
                        <div class="event-info-left">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                                class="bi bi-people" viewBox="0 0 16 16" aria-hidden="true">
                                <path
                ...
                <div class="share">
                    ...
                    <div class="share-list">
                        ...
                        <a id="fbShare" class="share-action share-icon" aria-label="Share on Facebook"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#4E5566"
                                class="bi bi-facebook" viewBox="0 0 16 16" aria-hidden="true">
                                <path
                                    d="M12 2.04V.5H9.75C8.26.5 7.5 1.5 7.5 2.83V4H6v2h1.5v6H10V6h1.5l.5-2H10V2.83C10 2.2 10.4 2 11 2H13v.04z" />
                            </svg>
                        </a>

                        <a id="xShare" class="share-action share-icon" aria-label="Share on X" href="#" target="_blank"
                            rel="noopener" title="Share on X">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#4E5566"
                        ...
                        <a id="emailShare" href="#" class="share-action share-icon" aria-label="Share via Email"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#4E5566"
                                ...
                                <path
                                    d="M6.761 8.83 0 12.803V14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-1.197l-6.761-3.973-1.239.744z" />
                                <path d="M10.197 8.32 16 4.697v7.104z" />
                            </svg>
                        </a>

                        <a id="waShare" href="#" class="share-action share-icon" aria-label="Share on WhatsApp"
                        ...
                        @if($isRegistered)
                            @if(!empty($event->maps_url))
                                <div class="text-center">
                                    <img src="{{ Storage::url($event->maps_url) }}" alt="Map"
        ...
        <section class="resource-box">
                ...
                <div class="resource-card {{ (isset($isRegistered) && $isRegistered && ((isset($attendanceSubmitted) && $attendanceSubmitted) || (!$eventIsFinished && (isset($eventStarted) && $eventStarted)))) ? '' : 'locked' }}" style="position:relative;">
                    ...
                    <div class="resource-value">
                        ...
                        @if(isset($attendanceSubmitted) && $attendanceSubmitted)
                            <p class="text-success" style="font-weight:600;">Berhasil Dilakukan</p>
                        @else
                            <p>Scan QR Event untuk absensi.</p>
                        @endif
                    </div>
                ...
                <div
                    ...
                    <div class="resource-value">
                        ...
                        @if(isset($isRegistered) && $isRegistered)
                            ...
                            @elseif($eventIsFinished)
                                <p>Tersedia setelah mengisi feedback.</p>
                            @else
                                <p>Tersedia setelah mengisi feedback.</p>
                            @endif
                        @else
            ...
            <div class="resource-card{{ !$isRegistered ? ' locked' : '' }}">
                    ...
                    <div class="resource-value">
                        <h6>Link Zoom</h6>
                        <p>{{ $isRegistered ? 'Tersedia setelah bookseat' : 'Tersedia setelah bookseat' }}</p>
                    </div>
            ...
            <div class="resource-card {{ ($isRegistered && $attendanceSubmitted) ? '' : 'locked' }}"
                    ...
                    <div class="resource-value">
                        ...
                        @if(isset($hasFeedback) && $hasFeedback)
                            <p class="text-success" style="font-weight:600;">Berhasil dilakukan</p>
                        @else
                            <p style="width: 70%;">Dibuka setelah acara selesai</p>
                        @endif
                    </div>
                                ...
                                <div class="feedback-rating-row" style="display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 15px;">
                                    ...
                                    <div style="flex: 1 1 140px; min-width: 0;">
                                        ...
                                        <div class="stars-rating-input" data-target="eventRating"
                                            style="letter-spacing: 2px; cursor: pointer; user-select: none; line-height: 1;">
 ...
 <div class="footer-section-wrapper">
                            ...
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="d-flex align-items-center mb-3" style="justify-content: flex-start;">
                                                        ...
                                                        <label class="form-check-label text-dark mb-0" for="confirm-eval">Saya yakin ingin mengirim feedback ini</label>
                                                    </div>
                                                </div>
    ...
    <script>
        // Lightweight flash toast (success) for this page
        (function () {
            ...
            function ensureStyles() {
                ...
                style.textContent = `
                        ...
                        .flash-toast{--flash-bg:#fff;--flash-border:#e5e7eb;--flash-color:#111827;--flash-accent:#16a34a;--progress-width:100;position:relative;display:flex;align-items:flex-start;gap:.75rem;padding:.9rem 1rem .95rem 1rem;border:1px solid var(--flash-border);background:linear-gradient(135deg,var(--flash-bg) 0%,#f9fafb 100%);border-radius:14px;box-shadow:0 8px 24px -8px rgba(0,0,0,.18),0 1px 3px rgba(0,0,0,.08);transform:translateY(-8px) scale(.96);opacity:0;transition:transform .45s cubic-bezier(.16,.8,.24,1),opacity .45s ease,box-shadow .3s ease;overflow:hidden;font-size:.875rem;color:var(--flash-color);}
                        ...
                        .flash-progress{position:absolute;left:0;bottom:0;height:3px;width:100%;background:rgba(22,163,74,.18);}
                        .flash-progress::after{content:'';display:block;height:100%;width:100%;transform-origin:left;transform:scaleX(1);background:rgba(22,163,74,.85);animation:flashProgress var(--flash-time,4.5s) linear forwards;}
                        @keyframes flashProgress{to{transform:scaleX(0);}}
                    `;
            ...
            function showSuccess(message, timeoutMs) {
                ...
                const time = typeof timeoutMs === 'number' ? timeoutMs : 4500;
                toast.style.setProperty('--flash-time', (time / 1000) + 's');
                toast.innerHTML = `
                    ...
                    `;
                toast.querySelector('.flash-message').textContent = message || 'Berhasil terdaftar';
            ...
            // Expose helper
            window.__idsporaFlashSuccess = function (message) {
                try { localStorage.setItem(STORAGE_KEY, message || 'Berhasil terdaftar'); } catch (_e) { }
            };

            // Consume on load (after reload)
            document.addEventListener('DOMContentLoaded', function () {
                let msg = null;
                try { msg = localStorage.getItem(STORAGE_KEY); localStorage.removeItem(STORAGE_KEY); } catch (_e) { }
                    ...
                    .then(async (r) => {
                        ...
                        if (ct.includes('application/json')) {
                            ...
                            if (data && (data.status === 'ok' || data.status === 'already')) {
                                try {
                                    const msg = data.status === 'already' ? 'Anda sudah terdaftar' : 'Berhasil terdaftar';
                                    if (typeof window.__idsporaFlashSuccess === 'function') window.__idsporaFlashSuccess(msg);
                                } catch (_e) { }
                                // Prefer server-provided redirect (toast will show after navigation)
