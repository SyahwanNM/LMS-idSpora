            ...
            <div class="participant-resources">
                ...
                <div class="resource-card {{ (!empty($event->vbg_path) && $isRegistered) ? '' : 'locked' }}">
                    ...
                    @if(!empty($event->vbg_path) && $isRegistered)
                        <a class="link-share" href="{{ $event->vbg_file_url }}"
                            download="{{ 'VBG_' . $event->id . '_' . strtoupper(str_replace(' ', '_', substr($event->title, 0, 20))) . '.png' }}"
        ...
        @if($isRegistered && $attendanceSubmitted)
            <div id="feedbackSection"
                ...
                <div class="row g-0" style="margin-top: 20px; min-height: 300px;">
                    ...
                    <div class="col-md-6"
                        style="background-color: #f8f9fa; padding: 1rem; border-right: 1px solid #e9ecef;">
                            ...
                            @if(isset($feedbacks) && $feedbacks->count() > 0)
                                @foreach($feedbacks as $feedback)
                                    <div class="rating-card mb-2"
                                        style="background: white; border: 1px solid #e9ecef; border-radius: 8px; padding: 0.75rem;">
                                        <div class="stars-rating mb-2">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    fill="{{ $i <= ($feedback->rating ?? 0) ? '#FFD600' : '#e0e0e0' }}"
                                                    viewBox="0 0 16 16" style="margin-right: 2px;">
                                                    <path
                                                        d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
                                                </svg>
                                            @endfor
                        ...
                        <form action="#" method="POST" id="feedback-form">
                            ...
                            <div class="mb-2">
                                <label class="form-label mb-1"
                                    style="font-weight: 500; color: #333; font-size: 0.9rem;">Rating Event</label>
                                <div class="stars-rating-input" data-target="eventRating"
                                    style="font-size: 1.5rem; letter-spacing: 4px; cursor: pointer; user-select: none;">
                            ...
                            <!-- Submit Button -->
                            <button type="button" id="submit-feedback-btn" class="btn w-100 fw-semibold"
                                style="background-color: #FFD600; color: #000; border: none; border-radius: 8px; padding: 0.6rem; font-size: 0.9rem;">
                                Submit Feedback
                            </button>
                        </form>
                    </div>
