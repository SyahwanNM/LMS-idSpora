...
<body>
    <div class="link-box mb-3 container" style="margin-top:80px;">
        <a href="{{ route('dashboard') }}">Home</a>
        <p class="d-inline mx-1">/</p>
        <a href="{{ route('events.index') }}">Event</a>
        <p class="d-inline mx-1">/</p>
        <span class="active">{{ Str::limit($event->title, 40) }}</span>
    </div>

    <div class="container-detail container mb-5">
        <div class="row g-4">
            <div class="col-lg-8 kiri">
                <img src="{{ $event->image ? Storage::url($event->image) : asset('aset/event.png') }}" class="event-img w-100 rounded" alt="{{ $event->title }}" style="object-fit:cover; max-height:420px;">
                <div class="profile-box d-flex align-items-center gap-3 mt-3">
                    <img src="{{ asset('aset/profile.png') }}" alt="profile" class="rounded-circle" width="60" height="60" style="object-fit:cover;">
                    <div>
                        <h5 class="mb-1">{{ $event->title }}</h5>
                        <h6 class="text-muted mb-0">Created by <span class="black"><b>IdSpora</b></span></h6>
                    </div>
                </div>
                <div class="overview mt-4">
                    <h3 class="mb-3">Overview</h3>
                    @php
                        // Hapus tag <p> pembungkus yang mungkin tersimpan dalam deskripsi agar tidak tampil sebagai teks literal
                        $cleanDescription = preg_replace('/<\\/?p>/i', '', $event->description ?? '');
                    @endphp
                    <div class="mb-0" style="white-space:pre-line;">{!! nl2br(e($cleanDescription)) !!}</div>
                </div>
                <div class="terms-condition mt-5">
                    <h3 class="mb-3">Terms and Condition</h3>
                    <p class="mb-0 small text-muted">Syarat & ketentuan default. (Tambahkan field khusus jika diperlukan nanti).</p>
                </div>
            </div>
            <div class="col-lg-4 kanan">
                <div class="price p-4 border rounded shadow-sm bg-white">
                    @php $hasDiscount = $event->hasDiscount(); @endphp
                    @if($hasDiscount)
                        <span class="text-muted text-decoration-line-through d-block">Rp{{ number_format($event->price,0,',','.') }}</span>
                        <h4 class="price-text mt-1 mb-2">Rp{{ number_format($event->discounted_price,0,',','.') }}</h4>
                        <div class="box-diskon d-flex align-items-center justify-content-between mb-2">
                            <div class="time-alert d-flex align-items-center gap-2">
                                ...
                                <p class="text-danger mb-0 small">Diskon {{ $event->discount_percentage }}%</p>
                            </div>
                            <small class="diskon badge bg-warning text-dark">{{ $event->discount_percentage }}% OFF</small>
                        </div>
                    @else
                        <h4 class="price-text mb-2">@if((int)$event->price===0) Gratis @else Rp{{ number_format($event->price,0,',','.') }} @endif</h4>
                    @endif
                    <hr>
                    <div class="info-box small">
                        <div class="date d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <p class="date-judul mb-0 fw-semibold">Tanggal</p>
                                <p class="date-text mb-0">{{ $event->event_date?->format('d F Y') ?? '-' }}</p>
                            </div>
                        </div>
                        <div class="time d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <p class="time-judul mb-0 fw-semibold">Waktu</p>
                                <p class="time-text mb-0">{{ $event->event_time?->format('H:i') ?? '-' }} WIB</p>
                            </div>
                        </div>
                        <div class="location d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <p class="location-judul mb-0 fw-semibold">Lokasi</p>
                                <p class="location-text mb-0">{{ $event->location ?? 'TBA' }}</p>
                            </div>
                        </div>
                        <div class="bahasa d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <p class="bahasa-judul mb-0 fw-semibold">Bahasa</p>
                                <p class="bahasa-text mb-0">Indonesia</p>
                            </div>
                        </div>
                        <div class="sertifikat d-flex align-items-center gap-2 mb-2">
                            ...
                            <div>
                                <p class="sertifikat-judul mb-0 fw-semibold">Sertifikat</p>
                                <p class="sertifikat-text mb-0">Gratis</p>
                            </div>
                        </div>
                    </div>
                    <hr>
                    ...
                    @auth
                        <button id="registerBtn" class="enroll btn w-100 mb-2 {{ $registered ? 'btn-success' : 'btn-primary' }}" {{ $registered ? 'disabled' : '' }} data-event-id="{{ $event->id }}">
                            {{ $registered ? 'Anda Terdaftar' : 'Daftar Sekarang' }}
                        </button>
                    @endauth
                    @guest
                        <a href="{{ route('login',['redirect'=>request()->fullUrl()]) }}" class="btn btn-primary w-100 mb-2">Login untuk Mendaftar</a>
                    @endguest
                    <button class="save btn btn-outline-secondary w-100 mb-3" type="button">Save</button>
                    <p class="note small text-muted mb-0">Note: all course have 30-days money-back guarantee</p>
                </div>
                <hr class="my-4">
                <div class="box-benefit p-4 border rounded bg-white shadow-sm">
                    <h4 class="mb-4">This Event Include</h4>
                    <div class="d-flex flex-column gap-3">
                        <div class="materi d-flex align-items-center gap-2">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#f4c430" class="ikon bi bi-book" viewBox="0 0 16 16"><path d="M1 2.828c.885-.37 2.154-.769 3.388-.893 1.33-.134 2.458.063 3.112.752v9.746c-.935-.53-2.12-.603-3.213-.493-1.18.12-2.37.461-3.287.811zm7.5-.141c.654-.689 1.782-.886 3.112-.752 1.234.124 2.503.523 3.388.893v9.923c-.918-.35-2.107-.692-3.287-.81-1.094-.111-2.278-.039-3.213.492zM8 1.783C7.015.936 5.587.81 4.287.94c-1.514.153-3.042.672-3.994 1.105A.5.5 0 0 0 0 2.5v11a.5.5 0 0 0 .707.455c.882-.4 2.303-.881 3.68-1.02 1.409-.142 2.59.087 3.223.877a.5.5 0 0 0 .78 0c.633-.79 1.814-1.019 3.222-.877 1.378.139 2.8.62 3.681 1.02A.5.5 0 0 0 16 13.5v-11a.5.5 0 0 0-.293-.455c-.952-.433-2.48-.952-3.994-1.105C10.413.809 8.985.936 8 1.783"/></svg>
                            <p class="materi-text mb-0">Materi pembelajaran Lengkap</p>
                        </div>
                        <div class="sertif d-flex align-items-center gap-2">
                            ...
                            <p class="sertif-text mb-0">Sertifikat Kehadiran</p>
                        </div>
                        <div class="record d-flex align-items-center gap-2">
                            ...
                            <p class="record-text mb-0">Rekaman Tersedia</p>
                        </div>
                        <div class="online d-flex align-items-center gap-2">
                            ...
                            <p class="online-text mb-0">100% Online Course</p>
                        </div>
                    </div>
                    <hr>
                    <div class="sponsor-box small">
                        <h5 class="mb-2">Sponsor & Partner</h5>
                        <p class="mb-1">Ko+Lab</p>
                        <p class="mb-1">Fakultas Ilmu Terapan</p>
                        <p class="mb-0">Telkom University</p>
                    </div>
                    <hr>
