...
<head>
    ...
    <style>
        ...
        .success-anim .check {
            ...
            animation: drawCheck 600ms 500ms ease-out forwards;
        }
    ...
    <div class="scan-container">
        ...
        <div class="scan-body">
            ...
            <div class="mt-3 d-flex align-items-center" style="gap:12px;">
                <label class="btn btn-outline-primary btn-sm mb-0 {{ !$canScan ? 'disabled' : '' }}" for="file-input"
                    title="Upload Photo to Scan" aria-label="Upload Photo to Scan">
                ...
                <input id="file-input" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml"
                    style="display:none;" @if(!$canScan) disabled @endif>

                <button id="start-btn" class="btn btn-outline-success btn-sm" type="button" @if(!$canScan) disabled
                @endif title="Start Camera" aria-label="Start Camera">
                    <i class="bi bi-camera-video"></i>
                    <span class="visually-hidden">Start Camera</span>
                </button>

                <button id="stop-btn" class="btn btn-outline-secondary btn-sm" type="button" disabled
                    title="Stop Camera" aria-label="Stop Camera">
                    <i class="bi bi-stop-circle"></i>
                    <span class="visually-hidden">Stop Camera</span>
                </button>

                <button id="swap-btn" class="btn btn-outline-secondary btn-sm" type="button" disabled
        ...
        (function () {
            ...
            async function persistAttendance(decodedText) {
                try {
                    ...
                    if (resp.ok) {
                        try {
                            localStorage.setItem('idspora_flash_success', 'SUCCESS. you have successfully done the attendance check today');
                        } catch (_) { }
